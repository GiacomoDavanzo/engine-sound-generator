...
class Engine {
  constructor({
    xAxisPowerValues = null,
    yAxisPowerValues = null,
    maxEngineBrakeTorque = 100,
    momentOfInertia = 1.0,
    engineFriction = 0.01,
    minRpm = 750.0,
    maxRpm = 7500.0,
    ...
    if ((xAxisPowerValues == null) !== (yAxisPowerValues == null)) {
      throw new Error(
        '"xAxisPowerValues" and "yAxisPowerValues" need both to be supplied.'
      );
    }
    ...
    this.powerValues = new SplineInterpolation(
      xAxisPowerValues,
      yAxisPowerValues
    );
  ...
  update(dt) {
    ...
    let exponent =
      0.5 + (1.5 * (this.rpm - this.minRpm)) / (this.maxRpm - this.minRpm);
    let powerPercentage = Math.pow(throttle, exponent);
    let omega = (this.rpm / 60.0) * 2 * Math.PI;
    let torqueEngine =
      powerPercentage *
        (this.powerValues.evaluate(this.rpm) / omega +
    ...
    this.rpm =
      ((angularMomentum / this.momentOfInertia) * 60.0) / (2 * Math.PI);
  }
}
