...
class LowpassFilter {
  constructor(frequency, lastValue = 0.0) {
    ...
    this.alpha =
      (2.0 * Math.PI * SAMPLING_RATE_INVERSE * frequency) /
      (2.0 * Math.PI * SAMPLING_RATE_INVERSE * frequency + 1.0);
...
class Cylinder {
  constructor({
    ...
    this.intakeWaveguide = new Waveguide(
      intakeWaveguideLength,
      0.01,
      intakeOpenReflectionFactor
    );
    this.exhaustWaveguide = new Waveguide(
      exhaustWaveguideLength,
      exhaustClosedReflectionFactor,
      0.01
    );
    this.extractorWaveguide = new Waveguide(
      extractorWaveguideLength,
      0.01,
      0.01
    );
  ...
  updateWaveguidesReflectionValues() {
    this.intakeWaveguide.reflectionFactorRight =
      this.intakeOpenReflectionFactor * this.intakeValve +
      this.intakeClosedReflectionFactor * (1.0 - this.intakeValve);
    this.cylinderWaveguide.reflectionFactorLeft =
      this.intakeOpenReflectionFactor * this.intakeValve +
      this.intakeClosedReflectionFactor * (1.0 - this.intakeValve);

    this.exhaustWaveguide.reflectionFactorLeft =
      this.exhaustOpenReflectionFactor * this.exhaustValve +
      this.exhaustClosedReflectionFactor * (1.0 - this.exhaustValve);
    this.cylinderWaveguide.reflectionFactorRight =
      this.exhaustOpenReflectionFactor * this.exhaustValve +
      this.exhaustClosedReflectionFactor * (1.0 - this.exhaustValve);
  }
  ...
  update(intakeNoise, straightPipeOutputLeft, x) {
    ...
    let currentCylinderAmplitude =
      this.pistonMotion * 1.5 + this.fuelIgnition * 5.0;
    ...
    this.extractorWaveguide.add(
      this.exhaustWaveguide.outputRight,
      straightPipeOutputLeft
    );
    this.exhaustWaveguide.add(
      cylinderOutputRight *
        (1.0 - 0 * this.exhaustWaveguide.reflectionFactorLeft),
      extractorOutputLeft
    );
    this.cylinderWaveguide.add(
      currentCylinderAmplitude +
        intakeOutputRight * (1.0 - this.intakeWaveguide.reflectionFactorRight),
      extractorOutputLeft * (1.0 - this.exhaustWaveguide.reflectionFactorLeft)
    );
    this.intakeWaveguide.add(
      intakeNoise,
      cylinderOutputLeft * (1.0 - this.intakeWaveguide.reflectionFactorRight)
    );
  }
...
class DelayLine {
  constructor(length) {
    if (length < 1) {
      throw new Error(
        "The length of the delay line needs to be a positive value."
      );
    }
  ...
  setValues(values, factor) {
    if (this.data.length != values.length) {
      throw Error(
        'The "values" array needs to have the same length as the delay line.'
      );
    }
...
class Waveguide {
  ...
  add(valueLeft, valueRight) {
    let reflectedValueLeft =
      this.lower.getAtPosition(0 * this.maxIndex) * this.reflectionFactorLeft;
    this.outputLeft =
      this.lower.getAtPosition(0 * this.maxIndex) *
      (1.0 - this.reflectionFactorLeft);

    let reflectedValueRight =
      this.upper.getAtPosition(0 * this.maxIndex) * this.reflectionFactorRight;
    this.outputRight =
      this.upper.getAtPosition(0 * this.maxIndex) *
      (1.0 - this.reflectionFactorRight);
...
class EngineSoundGenerator extends AudioWorkletProcessor {
  constructor(options) {
    ...
    this.updateParameters({
      cylinderCount: processorOptions.cylinders,
  ...
  updateParameters({
    ...
    straightPipeWaveguideLength,
    straightPipeReflectionFactor,

    mufflerElementsLength,
    action,

    outletWaveguideLength,
    outletReflectionFactor,
  }) {
    this.cylinders = [];
    ...
    this.straightPipe = new Waveguide(
      straightPipeWaveguideLength,
      straightPipeReflectionFactor,
      straightPipeReflectionFactor
    );
    this.muffler = new Muffler({
      elementLengths: mufflerElementsLength,
      action: action,
    });
    this.outlet = new Waveguide(
      outletWaveguideLength,
      outletReflectionFactor,
      outletReflectionFactor
    );
  ...
  _updateSample(index) {
    ...
    let intakeNoise = this.intakeNoiseLowPassFilter.getFilteredValue(
      2.0 * Math.random() - 1.0
    );
    if (this.rpm < 25.0) {
      intakeNoise = 0;
    ...
    let crankshaftValue = this.crankshaftLowPassFilter.getFilteredValue(
      0.25 * Math.random()
    );
    for (let i = 0; i < this.cylinders.length; i++) {
      let x =
        this.currentRevolution +
        i * (this.cylindersCountInverse + crankshaftValue);
      this.cylinders[i].update(
        intakeNoise,
        this.straightPipe.outputLeft,
        x % 1
      );
  ...
  process(inputList, outputList, parameters) {
    ...
    for (let i = 0; i < this.chunkSize; i++) {
      this.throttle =
        parameters["throttle"].length > 1
          ? parameters["throttle"][i]
          : parameters["throttle"][0];
      this.rpm =
        parameters["rpm"].length > 1
          ? parameters["rpm"][i]
          : parameters["rpm"][0];
    ...
    for (let i = 0; i < this.chunkSize; i++) {
      channelEngineBlockVibrations[i] =
        this.engineLowPassFilter.getFilteredValue(this.sound[i]);
    }
    return true;
  }
}
